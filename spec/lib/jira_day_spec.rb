...
describe JiraDay do
  ...
  context 'initialize' do
    it 'Should raise exception when search not 200' do
      ...
      allow(jira_request_double).to receive(:tickets_booked_time_to_day).with(test_date).and_return(response_double)
  ...
  context 'calculate_missing_time' do
    it 'Should calculate incomplete day' do
      ...
      allow(jira_request_double).to receive(:get_issue).with('KEY-01').and_return(response_double)

      issue2 = File.read('spec/test_data/issue_3_worklog_2019_04_01.json')
      response_double = double('response', code: 200, body: issue2)
      allow(jira_request_double).to receive(:get_issue).with('KEY-02').and_return(response_double)
      ...
      expect(test_obj).to receive(:print_booked_summary).once.and_call_original

      test_obj.calculate_missing_time

      expect(test_obj.date).to eq(test_date)
    ...
    it 'Should calculate complete day' do
      ...
      response_double = double('response', code: 200, body: issue1)
      allow(jira_request_double).to receive(:get_issue).with('KEY-03').and_return(response_double)
  ...
  context 'request_issue' do
    it 'Should raise exception when not 200' do
      ...
      expect { test_obj.request_issue('KEY-01') }.to raise_error("Response not 200, can't get issue: KEY-01")
    end
    ...
    it 'Should return json is 200' do
      ...
      expect(test_obj.request_issue('KEY-01')).to eq(JSON.parse('{"test": "yeah"}'))
    end
  end
  ...
  context 'populate_issues' do
    it 'Should populate array of issue' do
      ...
      expect(test_obj).to receive(:request_issue).with('KEY-01').once.and_call_original
      expect(test_obj).to receive(:request_issue).with('KEY-02').once.and_call_original
